...
## Тесты
domain - unit тесты валидации
application - покрыт интеграционными тестами через test-containers
posgres_repo - покрыт интеграционными тестами через test-containers
e2e - hurl (./tests)

## Описание решения
 Для тест окружения настроена автомиграция
 На уровне работы с бд реализована retry логика для mutable запросов - экспонециально + jitter
